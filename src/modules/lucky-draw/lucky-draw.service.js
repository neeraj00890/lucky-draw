...
function getBrownList(blackListNumbers) {
  const oneToThousand = Array.from({ length: NUMBER_THOUSAND }, (_, i) => i + 1);
...
async function handleGoldWinner(address, winningPrizes, randomNumber) {
  ...
  returnObject.winningType = WINNING_TYPES.GOLD;
      ...
      winningPrizes.availableGoldPrizes = winningPrizes.availableGoldPrizes.filter(goldPrizeNumber => goldPrizeNumber != randomNumber);
      winningPrizes.save();
      return returnObject;
}
...
async function handleSilverWinner(address, winningPrizes, randomNumber) {
  ...
  returnObject.winningType = WINNING_TYPES.SILVER;
      ...
      winningPrizes.availableSilverPrizes = winningPrizes.availableSilverPrizes.filter(silverPrizeNumber => silverPrizeNumber != randomNumber);
      winningPrizes.save();
      return returnObject;
}
...
async function handleBrownWinner(address, winningPrizes, randomNumber) {
  ...
  returnObject.winningType = WINNING_TYPES.BROWN;
      ...
      winningPrizes.availablebrownPrizes = winningPrizes.availablebrownPrizes.filter(brownPrizeNumber => brownPrizeNumber != randomNumber);
      winningPrizes.save();
      return returnObject;
}
...
exports.redeemPrize = async function (address) {
  ...
  if(totalAvailableNumbers.length) {
    ...
    const isGoldWinner = winningPrizes.availableGoldPrizes.includes(randomNumber);
    const isSilverWinner = winningPrizes.availableSilverPrizes.includes(randomNumber);
